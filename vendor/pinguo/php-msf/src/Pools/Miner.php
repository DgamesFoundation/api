...
class Miner
{
     ...
     * @return Miner
     */
    public function set($column, $value = null, $quote = null,$opt = self::EQUALS)
    {
        if (is_array($column)) {
...
//        var_dump('set UPdate测试',$statement);
    ...
    public function getSetString($usePlaceholders = true, $includeText = true)
    {
        ...
        foreach ($this->set as $set) {
            ...
            if ($usePlaceholders && $autoQuote) {
                $statement .= $set['column'] . " " . $set['opt'] . " ?, ";

                $this->setPlaceholderValues[] = $set['value'];
            }elseif ($set['opt']==self::ADD || $set['opt']==self::REDUCE){//更新修改20180710  添加 update table set field = field + 10 这种形式的更新
                $statement .= $set['column'] . " " . self::EQUALS . " " .$set['column'].$set['opt']. $this->autoQuote(
                        $set['value'],
                        $autoQuote
                    ) . ", ";
            }else {
                $statement .= $set['column'] . " " . $set['opt'] . " " . $this->autoQuote(
                    $set['value'],
                    $autoQuote
                ) . ", ";
            }
